...
> **Status: proof-of-concept.** The end-to-end loop works for **macOS/Linux** via a local runner process. VM isolation (Tart) is experimental. **Windows** is **opt-in** and **proven end to end on VMware Fusion** (2026-06-01): a `WindowsVMProvider` clones a throwaway Win11-ARM VM per job, the guest registers **outbound** to GitHub and runs **one** job, then powers off and the clone is destroyed. It's OFF by default behind a **"Set up Windows runner"** button (nothing heavy downloads or builds until you click it). The working backend is **VMware Fusion** (free since Nov 2024) — its `vmrun` CLI does headless clone/start/stop + snapshots and its EFI boots Win11-ARM, where stock QEMU's firmware hangs and UTM is GUI-bound. Fusion is a **manual download** (Broadcom portal — not brew-installable); the app detects it and installs the other free prerequisites (ISO-converter tools: `aria2`, `cabextract`, `wimlib`, `chntpw`) via Homebrew. It **never** installs **Parallels** (paid — used only if you already have it) and **never** installs Homebrew (if `brew` is missing it points you at https://brew.sh). The Swift `VMwareCLI` provider drives this per-job loop and "Go online" auto-selects Fusion when present; remaining is automating the one-time base build's few manual touches (VMware Tools, NIC, the boot-CD keypress). See [AGENTS.md](AGENTS.md) for the full picture, architecture, and roadmap.
