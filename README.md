...
3. **Go online.** Mactions downloads the runner agent on first use and brings ephemeral runners online. Reference them in a workflow with `runs-on: [self-hosted, macOS, mactions]` — or `runs-on: [self-hosted, Linux, ARM64, mactions]` for a Linux container runner (note the `ARM64`: it's an arm64 runner, so workflows opt in by label) — matching your labels.
...
The default macOS **local-process** runner has no isolation — only point it at **trusted / private** repos. **Linux** runs each job in a **throwaway container** (`--rm` discards the whole writable layer), which is clean for trusted/private repos but shares the host kernel; untrusted/public code wants a full VM (Tart/Lima for macOS/Linux, `WindowsVMProvider` on VMware Fusion for Windows — the VM paths for untrusted code are still experimental). The GitHub token is stored in a `0600` file under `~/.mactions` (not the keychain — an unsigned dev build re-prompts on every keychain access; a signed build could use the keychain).
