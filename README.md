...
> **Status: proof-of-concept.** The end-to-end loop works for **macOS/Linux** via a local runner process. VM isolation (Tart) is experimental. **Windows** is scaffolded and **opt-in** — a `WindowsVMProvider` clones a throwaway Win11-ARM VM per job (UTM/Parallels) and destroys it after. It's OFF by default behind a **"Set up Windows runner"** button (nothing heavy downloads or builds until you click it). Prerequisites are **free/OSS-first and installed by the app, not by hand**: a preflight checklist shows what's present (Homebrew, a hypervisor, the ISO-converter tools), and an **"Install free prerequisites"** button installs the *missing free* deps via Homebrew — **UTM** (free, open-source — the recommended default) plus `aria2`, `cabextract`, `wimlib`, `chntpw`. It **never** installs **Parallels** (paid — used only if you already have it) and **never** installs Homebrew (if `brew` is missing it points you at https://brew.sh). Setting up then auto-downloads the latest Win11 ARM64 ISO (UUP dump) and builds a one-time base image, and the app nudges you when a newer Windows build is out. Still **experimental and not yet live-verified end to end** (the free-deps install, the ISO auto-download/convert, the VM boot, and a green Windows job are unproven). See [AGENTS.md](AGENTS.md) for the full picture, architecture, and roadmap.
