...
> **Windows code signing (Azure Trusted Signing) on the ARM64 runner — OPEN (2026-06-02):** the full toolchain runs on the Win11-ARM runner (pwsh, `az`, OIDC login, .NET 10 all ✓) and `signtool` + the Azure CodeSigning dlib are invoked, but the final sign fails with `SignTool failed with exit code 3` (a swallowed data-plane rejection). The signing config/identity are **unchanged** from when it last worked (2026-05-23) — the only difference is the runner moved from hosted `windows-latest` (x64) to the self-hosted Win11-**ARM64** box, where the **x64** signtool/dlib run under emulation. Under investigation. See [AGENTS.md](AGENTS.md) → Windows support → *Windows code signing (Azure Trusted Signing) on ARM64*.
