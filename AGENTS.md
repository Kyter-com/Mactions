...
4. **bootstrap.ps1** (runs at FirstLogon, unattended): first installs **VMware Tools silently** from the attached CD (brings up the `vmxnet3` NIC — no in-box Win11-ARM driver) and waits for network; then installs the win-arm64 actions-runner agent → `C:\actions-runner`, Git (MinGit zip — no installer GUI), drops `C:\setup\run-job.ps1`, registers the `MactionsRunOnce` logon task, disables UAC, and `shutdown /s`.
...
- ~~Git for Windows `/VERYSILENT` shows the installer GUI on ARM64~~ — **FIXED**: `bootstrap.ps1` now installs Git via **MinGit** (`MinGit-*-arm64.zip`, a plain zip extracted with `ZipFile` + added to the machine PATH) — no installer UI at all.
...
- **Remaining (the one live-verify step):** a clean **app-driven** end-to-end run — the per-job shell loop is proven and the helper + provider are unit-tested + proven standalone, but the *automated base build* + the no-prompt ISO booting clean haven't been run live yet (the no-prompt ISO's El Torito EFI entry is structurally validated; a clean boot is pending). If the no-prompt ISO ever fails to boot, delete it and the build falls back to the prompting ISO.
...
- **Windows** support: VMware Fusion backend, wired into the UI (`WindowsVMProvider` + `VMwareCLI`, `WindowsImage`, `WindowsPreflight`, `scripts/prepare-windows-image` → `fusion-windows-base` + `mactions-fusion-vm`, the opt-in "Set up Windows runner" button + the preflight checklist/"Install free prerequisites" button + Windows toggle — see [Windows support](#windows-support)). The per-job ephemeral loop is **proven end to end on Fusion**; the prereq detection/brew-builder, UUP-dump auto-download, no-prompt ISO remaster, and build-id auto-update are implemented + unit-tested. **Remaining (NOT live-verified here):** a clean **app-driven** end-to-end run of the *automated base build* (the no-prompt ISO booting clean + the 25H2 OOBE handoff) and a green Windows job through the app.
