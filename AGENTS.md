...
Sibling docs: **[BASE.md](BASE.md)** (base-image philosophy: bake runner/OS semantics, leave convenience tools to workflows) and **[PARITY.md](PARITY.md)** (the per-OS contract vs GitHub-hosted runners — every verified difference and its workaround; update it when runner-visible behavior changes).
...
- 🟢 **Linux provider** (`LinuxContainerProvider`): **PROVEN END TO END (2026-06-08).** Through the real app UI (Linux tile → `setUpLinuxRunner` → "Go online"), a real ephemeral **arm64 container** runner registered **outbound** to GitHub and ran a **green** Linux job (`RUNNER_OS=Linux`, `RUNNER_ARCH=ARM64`, Ubuntu 24.04) on `PredictorSDK/PredictorSDK`, then auto-deregistered and the container was destroyed (`--rm`) — host left clean, and the fleet reconciled a fresh replacement while online. The substrate is a **throwaway container per job** (the cleanest ephemerality of the three OSes — the whole writable layer is discarded), not a VM: per job we `<cli> run --rm … /home/runner/run.sh` of the official multi-arch **`ghcr.io/actions/actions-runner`** image, delivering the JIT via the `ACTIONS_RUNNER_INPUT_JITCONFIG` env var (never the arg list), and the foreground process's exit code is the completion signal (so no power-state polling — it reuses `LocalProcessProvider`'s `terminationHandler` reaping). Backed by a **`LinuxContainerCLI`** abstraction with two conformers — **`DockerCLI`** (a `docker` CLI, typically **Colima**-managed; the free fallback, any macOS) and **`ContainerCLI`** (Apple's **`container`**, macOS 26+, one lightweight VM per container; the default when present). **Both free at any org size** (Colima = MIT, Apple `container` = Apache-2.0); Docker Desktop / OrbStack are detected-but-never-defaulted (paid for orgs at scale). Per **[BASE.md](BASE.md)** the image stays minimal — workflows install their own toolchain via `setup-*` (the proof job's Go came from `actions/setup-go`, not the base). See [Linux support](#linux-support).
...
- **`LinuxContainerProvider`** — the simplest of the three: a **container per job**, not a VM. `start()` runs `<cli> run --rm … /home/runner/run.sh` as a foreground `Process` (JIT via the `ACTIONS_RUNNER_INPUT_JITCONFIG` env, not the arg list), and the process's exit code IS the completion signal — so it reuses `LocalProcessProvider`'s `terminationHandler` reaping with **no boot/job/stop polling and no config-ISO** (those exist only because Fusion's VM shutdown is decoupled from the host process). Backed by a `LinuxContainerCLI` abstraction (pure command-builders, unit-tested) with **`DockerCLI`** (Colima/`docker`) and **`ContainerCLI`** (Apple `container`) conformers; `LinuxContainerProviderFactory.detectInstalledCLI()` prefers Apple `container` on macOS 26+ arm64, else a `docker`, else `nil`. The `--rm` writable-layer discard is the cleanest ephemerality of the three (cleaner than the macOS HOME-redirect clone). For **trusted/private repos** — containers share the host kernel, so route untrusted code to a VM (Tart/Lima). See [Linux support](#linux-support).
...
- **Linux guests:** easiest, and **implemented** as a **container per job** (`LinuxContainerProvider`) rather than a full VM — `--rm` gives the cleanest ephemerality. On Apple Silicon the container is **arm64**, so the fleet registers `[self-hosted, Linux, ARM64, mactions]` (the **`ARM64`** label is deliberate: `ubuntu-latest` is x64, so workflows must opt into the arm64 runner by label, and `RUNNER_ARCH=ARM64`). We pin `--platform linux/arm64` and never silently QEMU-emulate amd64 (push amd64 legs to hosted x64 via a matrix). A full Tart/Lima Linux **VM** is the fallback only when a job needs kernel-level isolation (untrusted/public code), kernel modules, or nested virt. See [Linux support](#linux-support).
...
sweep-collector signs its Windows installer with `azure/trusted-signing-action@v2`, and it now **signs green on a Mactions Win11-ARM runner** (proven in the v0.0.15 release, run `26923275454`). The `SignTool failed with exit code 3` that blocked this was **neither a data-plane/RBAC rejection nor raw emulation breakage**:
...
> ✅ **PROVEN END TO END (2026-06-08).** Through the real app UI, an ephemeral **arm64 container** runner registered **outbound** to GitHub and ran a **green** Linux job (`RUNNER_OS=Linux`, `RUNNER_ARCH=ARM64`, `aarch64`, Ubuntu 24.04.4, user `runner`) on `PredictorSDK/PredictorSDK`, then auto-deregistered and the container was destroyed (`--rm`) — host left clean, fleet reconciled a fresh replacement while online. Job's Go came from `actions/setup-go` (`go1.26.4 linux/arm64`), not the base image — exactly the [BASE.md](BASE.md) philosophy.
...
5. **Teardown** — `--rm` discards the container; `cleanup()` also force-`rm`s by name (a SIGKILL or a killed CLI client *can* orphan a container — confirmed live), and `HostCleanup.purgeStrayLinuxContainers()` reaps `label=mactions` leftovers on go-online (the analog of `purgeStrayWindowsClones`).

### Backends (`LinuxContainerCLI`, presence-detected like Fusion)

A pure command-builder protocol (`runArgs`/`stopArgs`/`rmArgs`/`inspectArgs`/`pullArgs`/`imageInspectArgs`/`daemonStatusArgs`/`daemonStartArgs`/`listByLabelArgs`) so the per-tool shapes are unit-testable without a live daemon. `detectInstalledCLI()` preference order — and **both defaults are free at any org size**, unlike Windows' single backend:

- **`ContainerCLI`** (Apple `container`, **macOS 26+**, arm64) — Apache-2.0, one lightweight VM per container (strongest isolation). **Validated live on 0.12.3 (macOS 26.5.1):** `run --rm/--name/--label/--cpus/--memory 6g/-e KEY` (env pass-through) + trailing `run.sh` all work. Installed via the **signed `.pkg` from github.com/apple/container/releases** (there is **no** Homebrew cask) at `/usr/local/bin/container`. Two backend-specific notes the code handles: (1) first use installs a **default Linux kernel** once (`system kernel set --recommended` — `system start` only prompts for it; gated since it's not idempotent), and (2) `container list` has **no `--filter`**, so the orphan sweep scopes by the `mactions-` name prefix instead of the label. Pre-1.0, so it's preferred only when present.
- **`DockerCLI`** (a `docker`, typically **Colima**-managed: `brew install colima docker`) — MIT/Apache, native `--vm-type vz` arm64, the version-agnostic free fallback for any macOS. Also matches a `docker` provided by Docker Desktop/OrbStack the user already has — but those are **never auto-recommended** (paid for orgs at scale: Docker Desktop >250 employees **and** >$10M rev; OrbStack ~all commercial use).

### Concurrency, labels, isolation

- **Cap** — `LinuxContainerBudget.maxConcurrentContainers` binds on the tighter of `(RAM_GB − 4) / 6` and `cores / 2`, with a hard `--cpus 2 --memory 6g` per container (Docker's default is unlimited → OOM risk). Far looser than the Windows VM RAM budget (sub-second start, shared kernel).
...
- **Isolation** — a fresh container is sufficient for **trusted/private repos** only (containers share the host kernel; GitHub warns self-hosted runners "should almost never be used for public repositories"). Untrusted/public/fork code wants a full VM (Tart/Lima). Docker-in-workflow (`jobs.<id>.container` / mounting `/var/run/docker.sock`) is **deferred** — off by default; it'd be a trusted-repo-only opt-in.
