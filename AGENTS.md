...
- **`ContainerCLI`** (Apple `container`, **macOS 26+**, arm64) — Apache-2.0, one lightweight VM per container (strongest isolation). Installed at `/usr/local/bin/container` (signed `.pkg`; `brew install --cask container`). Pre-1.0, so it's preferred only when present.
