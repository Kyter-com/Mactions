...
- **Hosted Windows OS-config parity is being added narrowly, recipe-stamped.** Recipe v5 sets LocalMachine PowerShell execution policy to `Unrestricted` so explicit `shell: powershell` steps work like hosted Windows; recipe v6 enables the OS-level `LongPathsEnabled=1` registry switch (`HKLM:\SYSTEM\CurrentControlSet\Control\FileSystem`) so post-checkout deep-path tooling behaves like hosted Windows. Git `core.longpaths` is deliberately untouched because the official hosted `Install-Git.ps1` does not set it either.
