...
- **GitHub-hosted parity packages — the UI picker (recipe v5, 2026-06-03).** The Windows pane has a **Packages** disclosure: one checkbox per item GitHub's own hosted **`windows-11-arm`** image ships (the menu mirrors `toolset-win-11-arm64.json` + the image's preinstalled tools in `actions/runner-images` — that image is the right compatibility target since our guest IS Win11-ARM). Two groups: **toolcache** entries (node 22/24, Python 3.12–3.14 arm64 + 3.13 x64-emulated, Go 1.22–1.25) installed exactly the way GitHub installs them (mirror of their `Install-Toolset.ps1`: read the `actions/*-versions` manifest, download the win32 archive, run its embedded `setup.ps1`, which writes the `<arch>.complete` marker `actions/setup-*` checks → checked versions resolve instantly per job at GitHub's exact versions), and **global tools** (gh CLI, CMake, .NET 8 SDK, Temurin Java 21/23, jq+yq — all ARM-native except jq, which is x64-under-emulation exactly as GitHub ships it). Plumbing: the selection rides `MACTIONS_WINDOWS_PACKAGES` (csv of catalog ids) → `prepare-windows-image` **awk-swaps the `$SelectedPackages` placeholder line in the STAGED copy** of `bootstrap.ps1` (anchor: `# MACTIONS-PACKAGES-LINE`; ids whitelisted `[a-z0-9.-]` so nothing can smuggle quoting; the repo file always keeps the empty default and the BOM/ASCII contract survives — awk only swaps one line far below the BOM) and records it at `~/.mactions/windows-base.packages` on success; the app compares recorded vs selected to nudge **"package selection changed — rebuild to apply"** (`MaintenanceReason.packagesChanged`, reuses the cached ISO). Selected packages are **FATAL on install failure + verified before the provisioning sentinel** (a base missing something the user checked must not snapshot — the v4 lesson). `RUNNER_TOOL_CACHE`/`AGENT_TOOLSDIRECTORY` are set machine-wide unconditionally (`C:\hostedtoolcache\windows`, matching GitHub). **MAINTAINER TOUCHPOINTS:** the catalog lives in `WindowsImage.packageCatalog` and MUST stay in lockstep with bootstrap's `$toolcacheCatalog` keys + `switch` cases — `testPackageCatalogMatchesBootstrapScript` fails on drift; when refreshing the menu against GitHub's image, re-read `toolset-win-11-arm64.json` and bump the recipe. Unchecked tools still work via `setup-*` at job time. Roadmap: generate the catalog from the toolset JSON at a pinned runner-images tag; a "match GitHub windows-11-arm" one-click preset; `rclone` (native win-arm64) as a catalog entry would let R2 uploads run on the Windows leg (wrangler can't — workerd ships no win32-arm64).
