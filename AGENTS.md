...
> ⚠️ **Live-verification finding (2026-06-01) — stock-QEMU-headless does NOT boot Win11-ARM.** A full base build was finally run end to end (after fixing a showstopper, below) and the headless QEMU install **hangs inside Microsoft's `bootaa64.efi`** — QEMU busy-spins a core at 99% CPU right after EDK2 hands off. Reproduced identically across **both** EDK2 firmwares (Homebrew `edk2-stable202408` and UTM's), **both** CPU models (`-cpu host`/`max`), and **with/without** `iommu=smmuv3` — so it's none of the knobs the scripts tune (keypress sprayer, iommu, CD topology); it's a **QEMU-build/HVF incompatibility** with Windows-ARM in stock Homebrew QEMU 11.0.1. UTM works only because it ships its **own patched QEMU** as `.framework` bundles (not invocable standalone — no `hypervisor` entitlement, needs UTM's Aqua/XPC launcher). **So the "fully-headless, free, no-GUI QEMU" premise of `QEMUCLI` is not achievable on this stack.** The realistic paths are **UTM** (patched QEMU that *does* boot Windows; free; needs an Aqua login session — which Mactions has; automatable via UTM's **AppleScript** interface since `utmctl` has no `create`) or **Parallels Pro** (authorized + fully headless CLI; **paid**, ~$120/yr). The genuine showstopper this surfaced — `build_unattend_fat`'s `mktemp`/`.dmg` bug that aborted every prior build before boot — is fixed (PR #9). See issue #5 for the full diagnostic log. Treat the QEMU-as-default text below as **superseded** by this finding.
